...
const AdminDashboard = () => {
  ...
  const isInstructor =
    userRole === 2 || userRole === 'instructor' || userRole === 'trial'
  ...
  useEffect(() => {
    if (!isInstructor) {
      dispatch(
        fetchRevenueAnalytics(revenueDateRange.from, revenueDateRange.to)
      )
    }
  ...
  const dashboardData = {
    ...
    ...levelStatistics,
    ...userStatus,
    totalCreatedPortfolios:
      levelStatistics.portfolioStatistics?.portfoliosCreated || 0,
    totalCompletedPortfolios:
      levelStatistics.portfolioStatistics?.totalCompletedPortfolios || 0,
    portfolioCompletionRate:
      levelStatistics.portfolioStatistics?.completionRate || 0
  }

  const { genderDistribution, ageDistribution, countryDistribution } =
    demographics
  ...
  const transformCountryDistributionData = () => {
    return Object.entries(countryDistribution || {}).map(
      ([country, count]) => ({
        label: country,
        value: count
      })
  ...
  const transformBrowserData = () => {
    ...
    return Object.entries(analyticsData.browserDistribution || {}).map(
      ([browser, count]) => ({
        label: browser,
        value: count,
  ...
  const transformDeviceData = () => {
    ...
    return Object.entries(analyticsData.deviceDistribution || {}).map(
      ([device, count]) => ({
        label: device,
        value: count,
  ...
  const transformReferralSourceData = () => {
    ...
    return Object.entries(analyticsData.referralDistribution || {}).map(
      ([source, count]) => ({
        label: source,
        value: count,
  ...
  const transformTrafficOriginData = () => {
    return Object.entries(analyticsData.trafficOrigin || {}).map(
      ([country, data]) => ({
        label: country,
        value: data.count
      })
  ...
  const calculateMaxYValue = (data) => {
    ...
    if (data[0]?.bars) {
      maxValue = Math.max(
        ...data.flatMap((group) => group.bars.map((bar) => bar.value))
      )
  ...
  const calculateHorizontalChartHeight = (
    itemsCount,
    rowHeight = 26,
    rowGap = 8,
    topPadding = 20,
    ...
    if (!itemsCount) return minHeight

    const barsHeight =
      itemsCount * rowHeight + Math.max(itemsCount - 1, 0) * rowGap
    const computedHeight = barsHeight + topPadding + bottomPadding + 24
              ...
              >
                <div style={{ flex: 1, width: '100%' }}>
                  ...
                  <HorizontalBarChart
                    ...
                    chartPadding={{ top: 20, right: 70, bottom: 40, left: 170 }}
                    barColor='#54C7DF'
                  />
                </div>
              </div>
