...
function LtsJournal(props) {
  ...
  const [currentPlaceholder, setCurrentPlaceholder] =
    useState('Select a Lesson')
  const [showSubscriptionModal, setShowSubscriptionModal] = useState(false)
  const user = useSelector((state) => state.user.user) // Add this line
  const [subscriptionModalparagraph, setSubscriptionModalparagraph] =
    useState('')
  ...
  const fetchLessons = async () => {
    try {
      ...
      if (response.data) {
        console.log('ridon144', response.data)
            ...
            transformedLessons[numericKey] = (response.data[key] || []).map(
              (lesson) => ({
                ...
                title:
                  ...
                  /^video\.[a-z]+(_[a-z]+)*$/.test(lesson?.video?.title || '')
                    ? lesson.title
                    : lesson?.video?.title,
  ...
  const handleLevelClick = (clickedLevel) => {
    ...
    if (!isInstructorUser && isInTrial) {
      if (clickedLevel === 1) {
        ...
        setSubscriptionModalparagraph(
          'To access Level 3, your free trial period must end and your subscription must be active. Your trial will automatically convert to a paid subscription.'
        )
      }
      setShowSubscriptionModal(true)
      return
    ...
    if (!isInstructorUser && clickedLevel > 0) {
      ...
      if (previousLevelLessons && previousLevelLessons.length > 0) {
        const allPreviousLessonsCompleted = previousLevelLessons.every(
          (lesson) => finishedContent.includes(lesson.id)
        )
        ...
        if (!allPreviousLessonsCompleted) {
          ...
          setLockModalMessage(
            `This level is currently locked. You must complete all lessons in Level ${levelNumber} before you can access Level ${currentLevelNumber}.`
          )
    ...
    const welcomeLessonId = getFirstNonSeparatorLesson(
      lessonsByLevel[clickedLevel]
    )
    if (welcomeLessonId) {
  ...
  useEffect(() => {
    ...
    const levelVideos = videos.filter((video) => {
      const levelRedirectIds =
        lessonsByLevel[activeLevel]?.map((lesson) => lesson.redirectId) || []
  ...
  const handleSaveAndContinue = async () => {
    ...
    try {
      ...
      const resolveLessonTitle = (lessonId) => {
        switch (lessonId) {
          case 51:
            return 'The Myths of Entrepreneurship'
          case 60:
      ...
      const navigateToNextLesson = async () => {
        ...
        if (nextLesson) {
          ...
          console.error(
            'Next lesson not found in lessonsByLevel:',
            nextLessonId,
            lessonsByLevel
          )
          toast.error(
            'Unable to find next lesson. Please try selecting it manually.'
          )
        }
      }
          ...
          <div className='row'>
            <div className='px-0'>
              <div>
                ...
                <div>
                  <div
                    className='gradient-background-journal course-in-entrepreneurship'
                          ...
                          if (index === activeLevel) {
                            ...
                            const isLevelAccessible =
                              ...
                              (index > 0 &&
                                ...
                                lessonsByLevel[index - 1].every((lesson) =>
                          ...
                          >
                            ...
                            >
                              ...
                              <p className='pt-3'>
                                ...
                                your values, your purpose, your mindset, and
                                your skillset. The first step in creating this
                                proof is developing content that solidifies your
                        ...
                        render={(renderProps) => {
                          ...
                          if (!isContentAccessible(journalId)) {
                            if (
                              lessonsLoading ||
                              !lessonsByLevel[0] ||
                              !Array.isArray(lessonsByLevel[0]) ||
                              lessonsByLevel[0].length === 0
                            ) {
                              return null
                            }
                            ...
                            return (
                              <Redirect
                                to={`${props.match.url}/${firstLessonId}`}
                              />
                            )
                          }
                              ...
                              noteButtonProps={{
                                ...
                                data: {
                                  id:
                                    ...
                                    renderProps.match.params.journalId,
                                  title:
                                    getLessonTitle(
                                      renderProps.match.params.journalId
                                    ) || 'My Note'
