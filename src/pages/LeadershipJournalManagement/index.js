...
const LeadershipJournalManagement = () => {
  ...
  const getSelectedJournalColor = () => {
    const selectedJournal = manageContentData.find(
  ...
  const handleViewEditJournal = async (journalItem, mode) => {
    try {
      ...
      if (journal.isSection) {
        // View section intro using CreateJournalTaskModal
        const taskData = {
          id: journal.id,
          title: journal.title,
          ...
          thumbnailUrl:
            journal.JournalImg?.url ||
            journal.video?.thumbnail ||
            journal.videos?.[0]?.thumbnail ||
            '',
      ...
      // For non-section items, use AddTaskModal to view the task
  ...
  const handleSaveAssignments = async (assignments) => {
    try {
      ...
      const updatePromises = validAssignments.map(async (assignment) => {
        try {
          console.log(
            `Assigning leadership journal ${assignment.journalId} to level ${assignment.levelId}`
          )
          ...
          console.error(
            `Error assigning leadership journal ${assignment.journalId}:`,
            individualError
          )
      ...
      toast.success(
        `${validAssignments.length} leadership journal(s) assigned successfully!`
      )
  ...
  const handleConfirmDeleteTask = async () => {
    try {
      await axiosInstance.delete(
        `/LtsJournals/${selectedTask.id}/delete-with-content`
      )
  ...
  return (
    <div className='leadership-journal-management'>
      <div>
        <div
          className='col-12 col-md-12 pe-0 me-0 d-flex-tab justify-content-between p-1rem-tab p-right-1rem-tab gap-4'
          style={{ backgroundColor: 'white' }}
        >
          <div
            className='d-flex justify-content-between flex-col-tab align-start-tab gap-5'
        ...
        <div className='header-tabs d-flex justify-content-between gap-3'>
          {levels.map((level, index) => (
            <button
              ...
              style={
                activeLevel === level
                  ? { backgroundColor: getSelectedJournalColor() }
                  : {}
              }
      ...
      <CreateJournalTaskModal
        ...
        taskData={editingTask}
        contentId={(() => {
          const foundItem = manageContentData.find(
            (item) => item.title === selectedCategory
          )
          return foundItem?.id
        })()}
