...
export default function StudentsSlider(props) {
  ...
  return (
    <div className='row mx-auto mt-2'>
      <div className='card-group desktop-menu ps-0 ms-0 d-flex justify-content-around mx-auto'>
        ...
        {students.length > 0 &&
          students?.slice(startIndex, endIndex).map((user, index) => (
            <StudentBox
              key={index}
              data={user}
        ...
        {students.length < endIndex &&
          [...Array(endIndex - students.length)].map((item, index) => (
            <div
        ...
        <div className='my-auto'>
          <button
            ...
            style={{
              width: '2%',
              visibility: students.length <= endIndex ? 'hidden' : 'visible'
            }}
          >
            <FontAwesomeIcon
              icon={faChevronRight}
      ...
      <div className='card-group mobile-menu justify-content-center'>
        ...
        <div className='text-center mx-3 d-flex'>
          ...
          {students.length < endIndex &&
            [...Array(endIndex - students.length)].map((key, index) => (
              <div
                className='col-auto my-connection-box empty-connection-box mx-2 p-3'
                key={index}
              ></div>
