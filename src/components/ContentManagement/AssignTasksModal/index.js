  ...
  const handleLevelChange = (journalId, levelId) => {
    setTaskAssignments((prevAssignments) =>
      prevAssignments.map((assignment) =>
        assignment.id === journalId
          ? { ...assignment, selectedLevel: levelId }
          : assignment
      )
    )
  }
    ...
    >
      <Modal.Body className='assign-tasks-modal-body'>
        ...
        <h5 className='modal-title'>
          Assign {getCategoryDisplayName()} to{' '}
          {type === 'content'
            ? 'Levels'
            : type === 'masterclass'
              ? 'Categories'
        ...
        {!loading && !error && (
          <div className='form-group'>
            <label className='form-label'>
              UNASSIGNED {getCategoryDisplayName().toUpperCase()}
            </label>

            {taskAssignments.length === 0 ? (
              <div className='text-center py-4'>
                <p>
                  No unassigned {getCategoryDisplayName().toLowerCase()} found.
                </p>
              </div>
            ) : (
                  ...
                  <div key={journal.id} className='task-item'>
                    ...
                    <div className='task-actions'>
                      <select
                        ...
                        onChange={(e) =>
                          handleLevelChange(journal.id, e.target.value)
                        }
                      >
                        <option value=''>
                          Select {type === 'masterclass' ? 'category' : 'level'}
                        </option>
                        {levels.map((level) => (
        ...
        {!loading && (
          <div className='modal-actions'>
            ...
            <button
              ...
              disabled={
                taskAssignments.filter((a) => a.selectedLevel !== '').length ===
                0
              }
            >
              SAVE AND CLOSE
