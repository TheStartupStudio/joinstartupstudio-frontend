    ...
    try {
      if (mode === 'organizations') {
        ...
        const organizations = results.map((org) => ({
          ...
          state: org.contry?.trim() || null,
          zipCode: org.zipCode?.trim() || null,
          learnerMonthlyPrice: org.learnerMonthlyPrice
            ? parseFloat(org.learnerMonthlyPrice)
            : 9.99,
          learnerYearlyPrice: org.learnerYearlyPrice
            ? parseFloat(org.learnerYearlyPrice)
            : 99.0
        }))
        ...
        const response = await axiosInstance.post(
          '/super-admin/organizations-csv/bulk-create',
          {
            organizations
          }
        )
        ...
        if (response.data.success) {
          ...
          toast.success(
            `${response.data.data.successCount} organization(s) added successfully!`
          )
        ...
        const learners = results.map((learner) => ({
          ...
          country: learner.country?.trim() || learner.state?.trim() || null,
          organization:
            learner.organization?.trim() || learner.universityId?.trim() || null
        }))

        console.log('📨 Final learners payload:', { learners })

        const response = await axiosInstance.post(
          '/super-admin/learners-csv/bulk-create',
          {
            learners
          }
        )
        ...
        if (response.data.success) {
          ...
          toast.success(
            `${response.data.data.successCount} learner(s) added successfully!`
          )
          ...
          if (
            ...
            response.data.data.errors.forEach((error) => {
              toast.error(
                `Row ${error.row} (${error.email || 'N/A'}): ${error.error}`,
                {
                  autoClose: 5000
                }
              )
    ...
    >
      <div className='bulk-add-learners-content'>
        ...
        >
          ...
          >
            <path
              d='M8 11L8 3M8 11L5 8M8 11L11 8'
              stroke='currentColor'
              strokeWidth='1.5'
              strokeLinecap='round'
            ...
            />
            <path
              d='M14 11V13C14 13.5523 13.5523 14 13 14H3C2.44772 14 2 13.5523 2 13V11'
              stroke='currentColor'
              strokeWidth='1.5'
              strokeLinecap='round'
              strokeLinejoin='round'
        ...
        <div
          ...
          onDrop={handleDrop}
          onClick={() =>
            !loading && document.getElementById('csvFileInput').click()
          }
        >
          <input
            type='file'
