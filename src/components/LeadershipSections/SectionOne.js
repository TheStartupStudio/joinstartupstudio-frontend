...
function SectionOne({ setIsReflection, contentData: propContentData, lessonId }) {
  ...
  // Get journal sections from API data
  const getJournalSections = () => {
    if (!contentData?.ltsJournalLevels) return []

    return contentData.ltsJournalLevels
      .filter(section => section.detailsText && section.published) // Only show sections with detailsText and published
      ...
      >
        {contentData?.introDescription ? (
          <>
            ...
            {getJournalSections().length > 0 && (
              <div className='accordion mt-3' id='progressAccordion'>
                {getJournalSections().map((section, index) => {
                  const sectionId = `section-${section.id}`
                  const headingId = `heading-${section.id}`
                  const collapseId = `collapse-${section.id}`

                  return (
                    <div key={section.id} className='accordion-item progress-details-accordion'>
                      <h2 className='accordion-header' id={headingId}>
                        <button
                          className={`accordion-button fw-medium ${openAccordion !== sectionId ? 'collapsed' : ''}`}
                          type='button'
                          onClick={() => handleAccordionClick(sectionId)}
                        >
                          {section.title}
                        </button>
                      </h2>
                      <div
                        id={collapseId}
                        className={`accordion-collapse collapse ${openAccordion === sectionId ? 'show' : ''}`}
                        aria-labelledby={headingId}
                      >
                        <div className='accordion-body d-grid gap-2'>
                            <div className='fs-15 fw-light'>
                              {parse(section.detailsText)}
                            </div>
                        </div>
                      </div>
                    </div>
